  ...
  return (
    <>
                ...
                <Container>  
                    ...
                    {allMessages.map( (allMessages, index) =>                              
                        <ListGroup.Item className={allMessages.sender === state.id ? "myMsgs" : "otherMsgs"} key={index}>
                            <p className='msgtxt' > <pre> {allMessages.message} </pre></p> 
                        </ListGroup.Item>                        
                    )}
                    </ListGroup>
                </Container>
      ...
      <Modal size="sm" show={smShowGroup} onHide={handleHideSelectionModalGroup} aria-labelledby="example-modal-sizes-title-sm" >
            ...
            <Modal.Body className='modalBody' style={{overflowY:'scroll'}} >
      ...
      <Modal size="lg" show={lgShowGroup} onHide={() => setLgGroupShow(false)} backdrop="static" keyboard={false} aria-labelledby="example-modal-sizes-title-lg">
        ...
        <Modal.Body className='modalBodyMsg' style={{maxHeight:'80vh',overflowY:'scroll'}}>
            {selectedId ?
            <>
                <Container>  
                    <ListGroup>                 
                    {allMessages.map( (allMessages, index) =>                              
                        <ListGroup.Item className={allMessages.sender === state.id ? "myMsgs" : "otherMsgs"} key={index}>
                            <p className='msgtxt' style={{overflowX:'unset',whiteSpace:'pre-wrap'}}>  {allMessages.message}  </p> 
                        </ListGroup.Item>                        
                    )}
                    </ListGroup>
                </Container>
        ...
        <Modal.Footer className='modalFooter'>
            {selectedId ?
                <Container>        
                    <InputGroup className="mb-3">
                        <Form.Control placeholder="Message" className='formInput' value={txtInput} onChange={(e) => setTxtInput(e.target.value)} onKeyDown={(e)=>{
                               
                                handleKeyDownGroup(e)
                            }} />
                        <Button variant="outline-secondary" className='searchBtn' id="button-addon2" disabled={stat==0?true:false} onClick={
                            ()=>{
                                setStat(0);
                                handleSendGroupBtn()
                            }
                      ...
                      {stat==0?<><Spinner animation="border" role="status">
      <span className="visually-hidden">Loading...</span>
    </Spinner> <Col style={{display:'inline',paddingBottom:'2px'}}>Loading</Col></>:null}
